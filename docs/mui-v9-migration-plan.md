...
Branch: `chore/mui-v9-migration`. Status as of this document: **Phases 1-11 complete and verified** (foundation, pilot, shared design tokens, first shared primitive, primitive rollout in progress — 14 of ~18 modals converted). Phase 12+ is scoped but not started — see "Remaining work" below.

**Related, out-of-branch:** three pre-existing app bugs found during Phase 9/10 live verification (report creation, Smart List creation, bulk-assign validation) were fixed on their own branch, unrelated to this migration — see [PR #15](https://github.com/siddhu6064/Nuatis/pull/15).
...
## Files added/changed (Phase 1-11)
...
This is a multi-week effort across 206 components; it was not attempted in one pass. Phase 3 (shared design tokens) and Phase 4 (first primitive, `Modal`) are done — see above. Phases 5-11 converted 13 more of the real dialog candidates found in Phase 5's triage (`lead-scoring`, `subscriptions`, `email-templates`, `DuplicatesReviewer`, `QuotePayments`, `InventoryList`, `settings/reports`, `settings/calendar`, dashboard `reports`, `campaigns/[id]`, `BulkActionBar`, `AppointmentsCalendar` ×2) — 14 of ~18 total converted, counting Phase 4's pilot. Suggested phasing from here:

1. **Continue the `Modal` rollout** — ~3 real dialog candidates remain (`reputation/VideoReviewsTab.tsx`, plus a couple not yet individually triaged). Two are explicitly held back on outward-facing side effects and need an explicit go-ahead: `quotes/payment-links` (live SMS) and `contacts/EmailComposeModal.tsx` (real Gmail/Outlook send, confirmed in Phase 7). `reputation/VideoReviewsTab.tsx` needs synthesized video-submission data before it can be verified live, same class of gap as Phase 6/7's synthesized test data. Each remaining conversion should keep following the established pattern: read the backend route before assuming risk level rather than pattern-matching on scary-looking words (Phase 7's `QuotePayments` had a "stripe" option that turned out to be pure record-keeping; Phase 8 confirmed `settings/calendar`'s risk was real, not assumed; Phase 9 hit a real, billed AI-generation call correctly blocked by the auto-mode classifier), real backend verification over render-without-error, a check for whether the modal has test data to exercise it against (Phase 6 synthesized a duplicate pair; Phase 7 used a quote's public accept endpoint; Phase 8 intercepted a fetch response when a modal's own precondition blocked the natural path to it; Phase 9 hit two independent pre-existing bugs that made live verification genuinely unreachable and fell back to structural inspection against already-verified sibling modals; Phase 10 disabled itself correctly against an opted-out contact before finding an opted-in one; Phase 11 drew a line between a one-line bug worth fixing on sight and a schema-level bug that needs a deliberate, non-mid-phase decision), and double-checking any `sx` color token actually exists in the theme rather than assuming a plausible-looking path resolves (Phase 8's `primary.50` typechecked fine while being silently wrong).
2. **A fourth pre-existing app bug, found during Phase 11's verification, out of scope for this migration:** `apps/api/src/routes/appointments.ts`'s `POST /block` never supplies `contact_id` on insert, but `appointments.contact_id` has been `NOT NULL` since the table's original migration (`0001_initial_schema.sql`) — the block-time feature (`0075_blocked_slots.sql`) never relaxed that constraint. Every "Block Time" submission has failed since the feature shipped. Needs a schema decision (relax the constraint vs. supply a placeholder contact), not a one-line fix — flagged rather than fixed mid-phase. (The three Phase 9/10 bugs — report `metric`/`metric_fn`, Smart List `created_by`, bulk-assign validation — were all one-line fixes and have already been fixed; see [PR #15](https://github.com/siddhu6064/Nuatis/pull/15).)
