...
Branch: `chore/mui-v9-migration`. Status as of this document: **Phases 1-13 complete and verified** (foundation, pilot, shared design tokens, first shared primitive, primitive rollout complete — 17 of ~18 modals converted, every candidate from Phase 5's original triage now done). Remaining work is `Drawer` (a different primitive) and opportunistic conversion — see "Remaining work" below.
...
## Files added/changed (Phase 1-13)
...
This is a multi-week effort across 206 components; it was not attempted in one pass. Phase 3 (shared design tokens) and Phase 4 (first primitive, `Modal`) are done — see above. Phases 5-13 converted every remaining dialog candidate found in Phase 5's triage (`lead-scoring`, `subscriptions`, `email-templates`, `DuplicatesReviewer`, `QuotePayments`, `InventoryList`, `settings/reports`, `settings/calendar`, dashboard `reports`, `campaigns/[id]`, `BulkActionBar`, `AppointmentsCalendar` ×2, `VideoReviewsTab`, `payment-links`, `EmailComposeModal`) — 17 of ~18 total converted, counting Phase 4's pilot. The `Modal` rollout is complete. Suggested phasing from here:

1. **All four pre-existing app bugs found during this migration's verification have now been fixed**, out of scope for the migration itself but tracked here for history: report `metric`/`metric_fn` mismatch, Smart List `created_by`, and bulk-assign validation (Phase 9/10, [PR #15](https://github.com/siddhu6064/Nuatis/pull/15)); blocked-appointment `contact_id` NOT NULL constraint (Phase 11, [PR #16](https://github.com/siddhu6064/Nuatis/pull/16), required a schema migration applied directly to the shared Supabase project with explicit go-ahead).
