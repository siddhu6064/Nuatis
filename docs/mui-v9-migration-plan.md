...
Branch: `chore/mui-v9-migration`. Status as of this document: **Phases 1-14 complete and verified** (foundation, pilot, shared design tokens, `Modal` rollout complete — 17 of ~18 modals converted — and `SlideOver`, a second shared primitive, now started — 1 of 4 edge-panel candidates converted). Remaining work is finishing the `SlideOver` rollout and opportunistic conversion — see "Remaining work" below.
...
## Files added/changed (Phase 1-14)
...
This is a multi-week effort across 206 components; it was not attempted in one pass. Phase 3 (shared design tokens), Phase 4 (first primitive, `Modal`, complete by Phase 13), and Phase 14 (second primitive, `SlideOver`, started) are done — see above. `Modal` covered every dialog candidate from Phase 5's original triage — 17 of ~18 counting Phase 4's pilot. `SlideOver` covers `InventorySlideOver` so far (1 of 4). Suggested phasing from here:

1. **Finish the `SlideOver` rollout** — 3 candidates remain: `StaffSlideOver`, `ShiftSlideOver`, `AppointmentDrawer`. `StaffSlideOver` and `ShiftSlideOver` already use the always-mounted+open-toggle pattern `SlideOver` assumes (confirmed in Phase 14), so converting them should be a direct swap plus dropping their own `if (!open) return null` guards, same as `InventorySlideOver`. `AppointmentDrawer` is the one with a real decision attached: its parent conditionally _mounts_ it rather than toggling `open`, so a like-for-like conversion works but won't get the slide animation — worth deciding at that point whether to also change the parent's mount pattern (a small change to `AppointmentsCalendar.tsx`, which this migration has already touched once, in Phase 11) or leave it as a known, disclosed gap. Read each backend route for outward-facing risk before converting, same discipline as every phase — `ShiftSlideOver` in particular manages staff shift assignments, worth confirming there's no notification/SMS side effect before assuming it's as safe as `InventorySlideOver` turned out to be.
