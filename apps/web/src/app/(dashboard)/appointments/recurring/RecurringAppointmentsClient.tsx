...
export default function RecurringAppointmentsClient({ contacts }: Props) {
  ...
  async function toggleEnabled(rule: Rule) {
    await fetch(`/api/recurring-appointments/${rule.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function remove(id: string) {
    await fetch(`/api/recurring-appointments/${id}`, { method: 'DELETE' })
    load()
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        ...
        >
          {showForm ? 'Cancel' : '+ New recurring appointment'}
        </Button>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border-brand">
            {rules === null ? (
              ...
              rules.map((r) => (
                <tr key={r.id}>
                  <td className="px-4 py-2.5 text-ink">{r.contacts?.full_name ?? '—'}</td>
                  <td className="px-4 py-2.5 text-ink">{r.title}</td>
                  <td className="px-4 py-2.5 text-ink3">
                    {scheduleLabel(r)} at {r.start_time}
                  </td>
