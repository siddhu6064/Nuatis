    ...
    <div>
      ...
      <div className="flex items-center gap-2 mb-6">
        {[1, 2, 3, 4].map((s) => (
          <div key={s} className="flex items-center gap-2">
            ...
            {s < 4 && <div className="w-8 h-px bg-bg3" />}
          </div>
        ))}
      ...
      {step === 1 && (
        <div
          ...
          className="border-2 border-dashed border-border-brand rounded-xl p-12 text-center hover:border-teal-400 transition-colors cursor-pointer"
          onClick={() => document.getElementById('csv-file-input')?.click()}
        >
          {loading ? (
            <p className="text-sm text-ink3">Parsing CSV...</p>
          ) : (
            <>
              ...
              <p className="text-xs text-ink4 mt-1">.csv files only, max 5MB</p>
            </>
          )}
          <input
            id="csv-file-input"
      ...
      {step === 2 && parseResult && (
        <div>
          ...
          <div className="overflow-x-auto mb-4">
            <table className="text-xs border border-border-brand rounded">
              <thead>
                <tr className="bg-bg">
                  {parseResult.headers.map((h) => (
                    <th key={h} className="px-3 py-1.5 text-left text-ink3 font-medium">
                      {mapping[h] !== 'skip' ? (
                        mapping[h]
                      ) : (
              ...
              <tbody>
                {parseResult.preview_rows.slice(0, 3).map((row, i) => (
                  <tr key={i} className="border-t border-border-brand">
                    {parseResult.headers.map((h) => (
                      <td key={h} className="px-3 py-1.5 text-ink3">
                        {row[h] ?? ''}
                      </td>
                    ))}
                  </tr>
      ...
      {step === 3 && parseResult && (
        <div>
          ...
          <div className="bg-bg rounded-lg p-4 mb-4 text-sm text-ink3 space-y-1">
            {Object.entries(mapping)
              .filter(([, v]) => v !== 'skip')
              .map(([csv, field]) => (
                <div key={csv}>
                  <span className="font-mono text-ink3">{csv}</span> {'\u2192'}{' '}
      ...
      {step === 4 && importResult && (
        <div>
          ...
          {!importResult.job_id && (
            <div className="bg-bg rounded-lg p-4 mb-4 text-sm space-y-1">
      ...
      {/* Import history */}
      <div className="mt-10">
        <h3 className="text-sm font-semibold text-ink2 mb-3">Import History</h3>
        {jobs.length === 0 ? (
          <p className="text-xs text-ink4">No imports yet</p>
        ) : (
          <div className="border border-border-brand rounded-lg overflow-hidden">
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-bg border-b border-border-brand">
                  <th className="text-left px-3 py-2 text-ink3 font-medium">File</th>
                  <th className="text-left px-3 py-2 text-ink3 font-medium">Date</th>
                  <th className="text-left px-3 py-2 text-ink3 font-medium">Rows</th>
                  <th className="text-left px-3 py-2 text-ink3 font-medium">Imported</th>
                  <th className="text-left px-3 py-2 text-ink3 font-medium">Skipped</th>
                  <th className="text-left px-3 py-2 text-ink3 font-medium">Errors</th>
                  <th className="text-left px-3 py-2 text-ink3 font-medium">Status</th>
                </tr>
              </thead>
              <tbody>
                {jobs.map((job) => (
                  <tr
                    key={job.id}
                    className="border-b border-border-brand last:border-0 hover:bg-bg cursor-pointer"
                    onClick={() => setExpandedJob(expandedJob === job.id ? null : job.id)}
                  >
                    <td className="px-3 py-2 text-ink2">{job.filename}</td>
                    ...
                    <td className="px-3 py-2 text-ink3">{job.row_count}</td>
