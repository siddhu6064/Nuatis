...
router.get('/', requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  const { data, error } = await supabase
    .from('gift_cards')
    .select('id, code, amount_cents, balance_cents, status, recipient_name, recipient_email, expires_at, created_at')
...
router.post('/', requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  if (!amount_cents || amount_cents <= 0) {
    res.status(400).json({ error: 'amount_cents required and must be > 0' }); return
  }
  const supabase = getSupabase()
  const { data, error } = await supabase
    .from('gift_cards')
  ...
  if (recipient_email) {
    console.log(`[gift-cards] gift card ${data.code} issued to ${recipient_email}, balance: ${amount_cents}`)
  }
...
router.post('/redeem', requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  if (!code || !amount_cents || amount_cents <= 0) {
    res.status(400).json({ error: 'code and amount_cents required' }); return
  }
  const supabase = getSupabase()
  ...
  if (card.balance_cents < amount_cents) {
    res.status(400).json({ error: 'Insufficient balance', balance_cents: card.balance_cents }); return
  }
...
router.get('/:code/balance', async (req: Request, res: Response): Promise<void> => {
  ...
  if (!code) { res.status(400).json({ error: 'code param required' }); return }
  const supabase = getSupabase()
  const { data, error } = await supabase
    .from('gift_cards')
    ...
    .maybeSingle()
  if (error) { res.status(500).json({ error: error.message }); return }
  if (!data) { res.status(404).json({ error: 'Gift card not found' }); return }
